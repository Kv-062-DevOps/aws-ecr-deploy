...
data "aws_iam_policy_document" "role-policy-document" {
  statement {
    ...
    principals {
      identifiers = var.aws_iam_role-principals
...
data "aws_iam_policy_document" "policy-document" {
  statement {
    ...
    resources = var.aws_iam_policy-resources

    actions = var.aws_iam_policy-actions
  }
}
...
resource "aws_iam_role_policy_attachment" "cross_account_assume_role" {
  ...
  role       = aws_iam_role.cross_account_assume_role[0].name
